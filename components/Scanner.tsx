...
const Scanner: React.FC<ScannerProps> = ({ onImport, embedded = false }) => {
  ...
  const handleAutoScan = async () => {
    ...
    try {
      // Run a battery of standard detection commands based on Platform
      const macCommands = [
        "java -version",
        "python3 --version",
        "python --version",
      ...
      const commands = platform === Platform.MAC ? macCommands : winCommands;

      for (const cmd of commands) {
        try {
          const output = await runCommand(cmd);
          if (output) combinedOutput += `\n$ ${cmd}\n${output}\n`;
        } catch (e) {
  ...
  const macCommands = [
    { label: "Java Versions", cmd: "/usr/libexec/java_home -V" },
    { label: "Brew Packages", cmd: "brew list --versions" },
    { label: "Python (System)", cmd: "python3 --version" },
